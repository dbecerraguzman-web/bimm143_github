...
```{r}
stat <- read.csv("Data Export Summary.csv")
stat
```


The commas in these numbers lead to the numbers here bein read as characters. 
...
>Q3: SKIP... Type HIV in the PDB website search box on the home page and determine how many HIV-1 protease structures are in the current PDB?
...
Write out our results as wee trajectory/movie of predicted motions:
![](ADK_M7.PDB.png)
...
This required a BioConductor package called "msa" that we need to install. 
...
We can make a visualization of the major conformational difference (i.e. large scale structural change) captured by our PCA analysis with the `mktrj()` function. 
