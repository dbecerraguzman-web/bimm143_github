...
We start by importing our data. It is a CSV file s0 we will use the
`read.csv()` function.
...
this for me machine learning models. We will use it later to compare our
...
0.4427 \>Q5. How many principal components (PCs) are required to
describe at least 70% of the original variance in the data?

PC3 \>Q6. How many principal components (PCs) are required to describe
...
that if cell nucleus are deeply indented (“concave”), irregular
non-circular (“compactness”), and have large “perimeter” values they
...
First scale the data `scale()`, then calculate the distance matrix
`dist()`. Then cluster with the `hclust()` function and plot:
...
You can claso use `cutree()` function with an argument `k=4` rather than
`h=height`.
...
In other words, our `wisc.pr$x` scored that we plotted above (the main
...
corespond to the expert `diagnosis`.
